...
`transWGS84` 页面目前只处理平面坐标：

```text
B/L 或 N/E -> 成都 X/Y
```

手工页面不再录入椭球高 H，也不再计算成都 H。
...
`transWGS84` 页面支持录入控制点并自动计算坐标转换参数。

每组控制点可填写：

```text
B / L / 国家 2000 N / 国家 2000 E / 成都 X / 成都 Y
```

其中：

```text
N/E/X/Y 用于计算平面转换参数 N0/E0/X0/Y0/a/b/d/e
B/L 和 N/E 用于计算经纬度投影到成果表 N/E 的校正量
```

当前平面参数计算规则：

```text
至少需要 3 组完整的 N/E/X/Y 控制点
第 1 组作为基准点 N0/E0/X0/Y0
第 2、3 组用于解 a/b/d/e
```
...
如果控制点不足 3 组，页面会回退到内置默认参数。

投影校正量计算规则：

```text
projectedN/projectedE = B/L 投影得到的原始国家 2000 N/E
dN = 成果表N - projectedN
dE = 成果表E - projectedE

页面使用所有完整投影控制点的 dN/dE 平均值。
```
...
```text
如果手动填写 N/E：
  使用手动 N/E -> 当前参数 -> 成都 X/Y

如果只填写 B/L：
  B/L -> 投影原始 N/E -> 投影校正 -> 当前参数 -> 成都 X/Y
```
